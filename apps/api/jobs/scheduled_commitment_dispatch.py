"""apps.api.jobs.scheduled_commitment_dispatch — Scheduled commitment dispatch KST cron.
...
- Audit-first INSERT `commitment_scheduled_dispatch_evaluated` +
  `commitment_report_dispatched` 2 NEW
...
# 4 cron expressions (PRD §F34.4-2 verbatim).
DISPATCH_CRON_EXPRESSIONS: dict[str, str] = {
    CommitmentDispatchSchedule.WEEKLY.value: "0 9 * * 1",  # KST Monday 09:00
    CommitmentDispatchSchedule.MONTHLY.value: "0 9 1 * *",  # KST 1st day of month 09:00
    CommitmentDispatchSchedule.QUARTERLY.value: "0 9 1 1,4,7,10 *",  # KST 1st day of quarter
    CommitmentDispatchSchedule.ANNUAL.value: "0 9 1 1 *",  # KST Jan 1 09:00
}

# Retry policy (PRD §F34.4-8 verbatim).
RETRY_BACKOFF_MINUTES: list[int] = COMMITMENT_DEFAULTS["retry_backoff_minutes"]
MAX_RETRY_COUNT: int = COMMITMENT_DEFAULTS["max_retry_count"]
...
def _check_idempotency(
    ...
    """Check dispatch idempotency (PRD §F34.4-7 verbatim).
    ...
    try:
        # Real check: query phase_18_finops_scheduled_commitment_dispatch for
...
def _dispatch_slack(
    ...
    """Dispatch commitment report to Slack (PRD §F34.4-4 verbatim).

    AD-14 stack pin: slack-sdk==3.23.0.
    """
    if dry_run:
        logger.info(
            "scheduled_commitment_dispatch.slack dry_run",
    ...
    try:
        ...
        logger.warning(
            "scheduled_commitment_dispatch.slack failed",
...
def _dispatch_email(
    ...
    """Dispatch commitment report via Email (PRD §F34.4-4 verbatim).

    AD-14 stack pin: sendgrid==6.11.0.
    """
    if dry_run:
        logger.info(
            "scheduled_commitment_dispatch.email dry_run",
            extra={"tenant_id": tenant_id, "report_id": report_id},
        )
        return True
    try:
        ...
        logger.warning(
            "scheduled_commitment_dispatch.email failed",
            extra={"tenant_id": tenant_id, "error": str(exc)},
        )
        return False
...
def _dispatch_ms_teams(
    ...
    """Dispatch commitment report via MS Teams (PRD §F34.4-4 verbatim).

    AD-14 stack pin: msgraph-sdk (Phase 18 NEW channel).
    """
    if dry_run:
        logger.info(
            "scheduled_commitment_dispatch.ms_teams dry_run",
            extra={"tenant_id": tenant_id, "report_id": report_id},
        )
        return True
    try:
        # Real MS Teams dispatch via MS Graph API (Phase 18 NEW).
        return True
    except Exception as exc:
        logger.warning(
            "scheduled_commitment_dispatch.ms_teams failed",
            extra={"tenant_id": tenant_id, "error": str(exc)},
        )
        return False
...
def _dispatch_s3_archive(
    ...
    """Archive commitment report to S3 (PRD §F34.4-4 verbatim).

    AD-14 stack pin: boto3 S3 client.
    """
    if dry_run:
        logger.info(
            "scheduled_commitment_dispatch.s3 dry_run",
            extra={"tenant_id": tenant_id, "report_id": report_id},
        )
        return True
    try:
        # Real S3 archive via upload_commitment_report (Phase 18 EXTENSION).
        return True
    except Exception as exc:
        logger.warning(
            "scheduled_commitment_dispatch.s3 failed",
            extra={"tenant_id": tenant_id, "error": str(exc)},
        )
        return False
...
def _lifecycle_state_machine(
    ...
    """Dispatch lifecycle state machine (PRD §F34.4-6 verbatim).
...
def schedule_commitment_dispatch(
    tenant_id: str,
    ...
    """Schedule commitment dispatch (PRD §F34.4-1 verbatim).

    Phase 18 wire (cj-style 135번째) — main entry.
    ...
    Raises:
        CommitmentCronExpressionInvalidError, CommitmentDispatchIdempotencyViolationError,
        CommitmentRecipientResolverError, ScheduledCommitmentDispatchError.
    """
    ...
    recipients = {
        ...
        "owner_only": recipient_strategy == CommitmentRecipientStrategy.OWNER_ONLY.value,
    }

    # Dispatch lifecycle state machine.
    initial_status = "scheduled"

    # Audit-first INSERT `commitment_scheduled_dispatch_evaluated` (CR 1-1).
    if not dry_run:
        try:
            ...
            if report_id:
                emit_audit_typed(
                    action="commitment_report_dispatched",
                    tenant_id=tenant_id,
                    actor_id=actor_id,
                    trace_id=trace_id,
                    resource_id=dispatch_id,
    ...
    logger.info(
        "scheduled_commitment_dispatch.schedule_commitment_dispatch",
        extra={
            "tenant_id": tenant_id,
            "dispatch_id": dispatch_id,
            "dispatch_schedule": dispatch_schedule,
...
def _compute_period_key(dispatch_schedule: str) -> str:
    ...
    if dispatch_schedule == CommitmentDispatchSchedule.WEEKLY.value:
        return now_kst.strftime("%Y-W%V")
    if dispatch_schedule == CommitmentDispatchSchedule.MONTHLY.value:
        return now_kst.strftime("%Y-%m")
    if dispatch_schedule == CommitmentDispatchSchedule.QUARTERLY.value:
        quarter = (now_kst.month - 1) // 3 + 1
        return f"{now_kst.year}-Q{quarter}"
    if dispatch_schedule == CommitmentDispatchSchedule.ANNUAL.value:
