...
+ Story 12.3 — tenant hard-delete cron (PRD §F12.3 + NFR4 2절).
...
- Pure orchestration only; business logic lives in the relevant service
  ...
  + `apps.api.modules.m12_account.services.account_deletion_service`).
- Idempotent — re-running with the same `now` is a no-op.

Active cron jobs (Story 12.3 wire):
...
- `tenant_hard_delete` — daily 04:00 KST (UTC 19:00), per-tenant 30-day
  pending_deletion → deleted hard-delete sweep (Story 12.3).
"""
