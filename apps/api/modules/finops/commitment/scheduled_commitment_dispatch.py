"""apps.api.modules.finops.commitment.scheduled_commitment_dispatch — Commitment dispatch scheduler.

Phase 18 wire (cj-style 135번째) — FinOps Cloud Commitment Management
territory (PRD §F34.4 verbatim + AD-45 (d) decision).
...
Functions:
- `schedule_commitment_dispatch` — main entry (PRD §F34.4-1 verbatim)
...
- `dispatch_commitment_report` — Slack + Email + MS Teams + S3 archive dispatch
- `validate_commitment_dispatch` — pure validator (CR 11-4 P-015 verbatim)

TypedDict:
- `ScheduledCommitmentDispatch` — see apps.api.modules.finops.commitment.serializers

Exceptions (CR 12-5 D-14 envelope):
- `ScheduledCommitmentDispatchError` (500)
- `CommitmentCronExpressionInvalidError` (400)
- `CommitmentRecipientResolverError` (404)
- `CommitmentDispatchIdempotencyViolationError` (422)

CR lessons applied:
- CR 0-2 RLS — tenant_id selector + multi-tenant isolation.
- CR 1-1 audit-first INSERT — `commitment_scheduled_dispatch_evaluated`
  + `commitment_report_dispatched` AFTER dispatch.
...
# Cron expression map (KST = UTC+9). Phase 18 wire (cj-style 135번째) —
# 4 schedules aligned with Phase 16/17 dispatch pattern verbatim.
...
_RECIPIENT_TEMPLATES: dict[str, dict[str, Any]] = {
    "owner_only": {
        "slack_channels": ["#bizup-commitment-owner"],
        "email_recipients": ["tenant_owner@bizup.kr"],
        "ms_teams_channels": ["commitment-owner"],
        "s3_archive_enabled": True,
    },
    "commitment_team": {
        "slack_channels": ["#bizup-commitment-team"],
        "email_recipients": ["commitment_team@bizup.kr"],
        "ms_teams_channels": ["commitment-team"],
        "s3_archive_enabled": True,
    },
    "finance_team": {
...
def _compute_dispatch_cache_key(
    ...
    """Compute SHA-256 cache key for ScheduledCommitmentDispatch."""
    payload = f"{tenant_id}:{dispatch_schedule}:{period_key}:commitment_dispatch"
...
def _validate_inputs(
    ...
    if dispatch_schedule not in ALL_COMMITMENT_DISPATCH_SCHEDULES:
        raise CommitmentCronExpressionInvalidError(
            reason=f"invalid_schedule:{dispatch_schedule}",
            tenant_id=tenant_id,
            allowed=list(ALL_COMMITMENT_DISPATCH_SCHEDULES),
        )
    if recipient_strategy not in ALL_COMMITMENT_RECIPIENT_STRATEGIES:
        raise CommitmentRecipientResolverError(
            reason=f"invalid_strategy:{recipient_strategy}",
            tenant_id=tenant_id,
            allowed=list(ALL_COMMITMENT_RECIPIENT_STRATEGIES),
        )
    if not period_key:
        raise CommitmentCronExpressionInvalidError(
...
def resolve_cron_expression(dispatch_schedule: str) -> str:
    ...
    Phase 18 wire (cj-style 135번째) — returns apscheduler-compatible
    ...
    Raises:
        CommitmentCronExpressionInvalidError — Unknown schedule (400).
    """
...
def resolve_recipient_list(
    ...
    Phase 18 wire (cj-style 135번째) — returns recipient_list JSONB.
    ...
    Raises:
        CommitmentRecipientResolverError — Resolution failure (404).
    """
...
def dispatch_commitment_report(
    tenant_id: str,
    dispatch_schedule: str,
    period_key: str,
    recipient_strategy: str,
    ...
    """Schedule + dispatch commitment report via KST cron.

    Phase 18 wire (cj-style 135번째) — main entry (PRD §F34.4-1 verbatim).
    ...
    Args:
        ...
        recipient_strategy: owner_only/commitment_team/finance_team/custom_recipients.
        trace_id: Trace ID for audit (CR 1-1 ContextVar).
        db_session: Optional DB session (None for dry-run).

    Returns:
        ScheduledCommitmentDispatch TypedDict 10 fields.

    Raises:
        CommitmentCronExpressionInvalidError — Invalid schedule (400).
        CommitmentRecipientResolverError — Resolution failure (404).
        CommitmentDispatchIdempotencyViolationError — Duplicate dispatch (422).
        ScheduledCommitmentDispatchError — apscheduler failure (500).
    """
    ...
    # must be unique (PRD §F34.4-10 verbatim).
    if db_session is not None:
        try:
            # Real DB path: query scheduled_commitment_dispatch table
            ...
            if existing is not None:
                raise CommitmentDispatchIdempotencyViolationError(
                    reason="duplicate_dispatch",
                    tenant_id=tenant_id,
                    dispatch_schedule=dispatch_schedule,
                    period_key=period_key,
                )
        except CommitmentDispatchIdempotencyViolationError:
            raise
        except ImportError:
            pass
        except Exception as exc:
            logger.warning(
                "scheduled_commitment_dispatch.dispatch_commitment_report idempotency_check failed",
    ...
    dispatch: ScheduledCommitmentDispatch = {
    ...
    if db_session is not None:
        try:
            ...
            # and persists job_id to scheduled_commitment_dispatch table.
            logger.info(
                "scheduled_commitment_dispatch.dispatch_commitment_report apscheduler_registered",
                extra={
                    "tenant_id": tenant_id,
                    "dispatch_schedule": dispatch_schedule,
                    "cron_expression": cron_expression,
            ...
            logger.warning(
                "scheduled_commitment_dispatch.dispatch_commitment_report apscheduler_failed",
                extra={"tenant_id": tenant_id, "error": str(exc)},
            )
            raise ScheduledCommitmentDispatchError(
                reason=str(exc),
                tenant_id=tenant_id,
            ) from exc

    # Audit-first INSERT — `commitment_scheduled_dispatch_evaluated`
    # (CR 1-1 verbatim).
    if db_session is not None:
        try:
            ...
            emit_audit_typed(
                ...
                action="commitment_scheduled_dispatch_evaluated",
    ...
    logger.info(
        "scheduled_commitment_dispatch.dispatch_commitment_report",
        extra={
            "tenant_id": tenant_id,
            "dispatch_schedule": dispatch_schedule,
            "recipient_strategy": recipient_strategy,
...
def validate_commitment_dispatch(
    dispatch: ScheduledCommitmentDispatch,
) -> bool:
    """Pure validator for ScheduledCommitmentDispatch TypedDict.
    ...
    if dispatch["dispatch_schedule"] not in ALL_COMMITMENT_DISPATCH_SCHEDULES:
        raise CommitmentCronExpressionInvalidError(
            reason=f"invalid_schedule:{dispatch['dispatch_schedule']}",
            tenant_id=str(dispatch.get("tenant_id", "")),
        )
    if dispatch["recipient_strategy"] not in ALL_COMMITMENT_RECIPIENT_STRATEGIES:
        raise CommitmentRecipientResolverError(
