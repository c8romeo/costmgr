...
describe("1st release support parity", () => {
  ...
  it("AC3.2 — OnboardingTooltip.tsx has 4 tooltip conditions (SSOT useTranslations)", () => {
    ...
    expect(content).toContain("step: 1 | 2 | 3 | 4");
    // D-EPIC-16-REVIEW-DEFER-6 (L11) RESOLVED — tooltips now use
    // useTranslations SSOT (CR 11-4 D-002 + P-015). Verify i18n keys
    // map to all 4 step values, NOT hardcoded Korean literals.
    expect(content).toContain('useTranslations("onboarding")');
    expect(content).toContain("tooltip_dashboard");
    expect(content).toContain("tooltip_data");
    expect(content).toContain("tooltip_reports");
    expect(content).toContain("tooltip_security");
  });
  ...
  it("AC3.5 — ko-KR.json has onboarding namespace", () => {
    ...
    // D-EPIC-16-REVIEW-DEFER-6 (L11) RESOLVED — tooltip_* SSOT keys
    // added so OnboardingTooltip.tsx can load via useTranslations.
    expect(ko.onboarding.tooltip_dashboard).toBe(
      "대시보드 위젯 5종을 자유롭게 추가/제거하세요",
    );
    expect(ko.onboarding.tooltip_data).toBe(
      "ABC/TDABC 분석을 위한 원가 데이터를 입력하세요",
    );
    expect(ko.onboarding.tooltip_reports).toBe(
      "월간/분기/연간 보고서를 자동 생성합니다",
    );
    expect(ko.onboarding.tooltip_security).toBe("TOTP 앱으로 2차 인증을 설정하세요");
  });
