/**
 * apps/web/lib/report21-pdf.ts — Story 9.4
 *
 * TS mirror for Report #21 PDF download (POST /api/v1/reports/21/pdf).
 *
 * A30 forward-lock SHARED PDF generator 결정 wire (9-3 handoff lock):
 * Report #21 (Cost Object Breakdown) 본 진입점 + Report #15 (활동원가
 * 내역서) 후속 = SHARED factory pattern via Discriminated union
 * report_id: Literal[15..21].
 */

import { REPORT21_ERROR_CODES, type Report21ErrorCode } from "./report21";

// Re-export Report21ErrorCode so callers (PdfExportButton) can use it as a
// React state type without importing from ./report21 directly (SSOT).
export type { Report21ErrorCode };

export interface Report21PdfResponse {
  period_key: string;
...
/**
 * Generate Report #21 PDF via POST /api/v1/reports/21/pdf.
 *
 * Returns discriminated union: PDF envelope (success) | Error envelope.
 */
export async function downloadReport21PdfTS(
  periodKey: string,
  accessToken: string | undefined,
): Promise<
  | { kind: "ok"; data: Report21PdfResponse }
...
> {
  ...
  try {
    ...
    const response = await fetch("/api/v1/reports/21/pdf", {
      method: "POST",
      headers,
    ...
    if (!response.ok) {
      ...
      return {
        kind: "error",
        code: (errBody.code as Report21ErrorCode) ||
          REPORT21_ERROR_CODES.PDF_GENERATION_ERROR,
        message: errBody.message_ko || `HTTP ${response.status}`,
      };
    }
    const data = (await response.json()) as Report21PdfResponse;
    return { kind: "ok", data };
  } catch (e) {
...
/**
 * Trigger browser PDF download with filename `report21_{periodKey}.pdf`.
 */
export function triggerPdfDownload(pdfBlob: Blob, periodKey: string): void {
  const url = URL.createObjectURL(pdfBlob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `report21_${periodKey}.pdf`;
