...
import type { CompletionStatus } from "./api-client";
...
export async function fetchCompletionServerSide(
  ...
  try {
    const res = await fetch(`${apiBaseUrl()}/api/v1/tenant-settings/completion`, {
      method: "GET",
      headers,
      cache: "no-store",
    });
    ...
    // Network / decode errors are non-fatal at RSC render time — let
    // the Client Component's polling take over.
    return null;
  }
}
