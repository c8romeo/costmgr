...
| **H8**: AC7.4 spec file rename variance — `test_epic_16_saml_routes_extended.py` → actual `test_epic_16_tenant_idp_lookup.py` | (a) docs 정합 | Spec 회기 update — 기능적으로 similar coverage (lookup module + saml_routes integration smoke). PRD §F19.7 AC7.4 filename 정합 update 필요 | ✅ **RESOLVED by cj-style 78번째 wire (2026-08-22)** — spec AC7.4 line 90 updated to actual filename `test_epic_16_tenant_idp_lookup.py` (~+19 pytest cases, 6 test classes verbatim bind). D-EPIC-16-REVIEW-DEFER-2 ✅ RESOLVED. |
| **M5**: `audit_action.py` typo risk — emit_audit_typed가 frozenset validation 없이 action name 통과시킴 | (b) retro input | CR 1-1 lesson carry. 1차 출시 후 결정 (Epic 17+ 또는 별도 epic) | ✅ **RESOLVED by cj-style 78번째 wire (2026-08-22)** — added `TestTypoActionRejected` to `tests/api/core/test_epic_16_audit_log_verification.py` (5 parametrized typo scenarios + 1 unknown ActionClass scenario, 6 NEW pytest cases). Validation hardening pin: emit_audit_typed() MUST raise ValueError on invalid action OR unknown ActionClass (CR 1-1 verbatim lesson). D-EPIC-16-REVIEW-DEFER-3 ✅ RESOLVED. |
| **M7**: acme seed URL placeholder deviation (actual `idp.example.com` vs spec verbatim `idp.acme.com`) | (a) docs 정합 | Epic 15 backward-compat 우선 (Epic 15 wire 의 hardcoded `acme` placeholder 보존 결정). Atomic sprint 한계 인정. PRD §F19.5 AC5.3 정합 sweep 결정 | ✅ **RESOLVED by cj-style 78번째 wire (2026-08-22)** — spec AC5.3 line 73 updated to actual `idp_entity_id='https://idp.example.com/sso'` + `idp_sso_url='https://idp.example.com/sso?tenant=acme'` per Epic 15 wire `5f9e37f` line 94 backward-compat 정합 결정. Migration `apps/api/alembic/versions/0038_epic_16_tenant_idps.py:71-78` `_ACME_*` 상수 verbatim bind preserved. D-EPIC-16-REVIEW-DEFER-4 ✅ RESOLVED. |
| **M9**: AC7.2 routes test count underrun (19 vs spec ~25) — coverage gap | (b) retro input | Epic 16 close-out retro (cj-style 72번째) 진입 시점에 A104 결정. RLS multi-tenant isolation + audit-first INSERT 검증 보강 | ✅ **RESOLVED by cj-style 78번째 wire (2026-08-22)** — added 8 NEW CRUD route contract tests (TestCreateRouteContract: 3 + TestUpdateRouteContract: 2 + TestDeleteRouteContract: 1 + TestTestRouteContract: 2) to `tests/api/core/test_epic_16_idp_admin_routes.py`. Actual count 19 → 27 (exceeds spec target 25). RLS multi-tenant isolation + audit-first INSERT 검증 보강 보존 (separate files: test_epic_16_tenant_idp_lookup.py + test_epic_16_audit_log_verification.py). D-EPIC-16-REVIEW-DEFER-5 ✅ RESOLVED. |
| **L11**: `OnboardingTooltip.tsx` removed `step_dashboard_title` stale i18n key may persist in `ko-KR.json` | (a) docs 정합 | P-015 ko-KR.json SSOT drift detector sweep 결정 (cj-style carry-over pattern) | ✅ **RESOLVED by cj-style 78번째 wire (2026-08-22)** — (1) added 4 NEW i18n keys `onboarding.tooltip_{dashboard,data,reports,security}` to `apps/web/messages/ko-KR.json` (P-015 SSOT discipline) / (2) refactored `apps/web/components/onboarding/OnboardingTooltip.tsx` to use `useTranslations("onboarding")` instead of hardcoded Korean literals (CR 11-4 D-002 verbatim lesson) / (3) updated `apps/web/__tests__/1st-release/support-parity.test.ts` AC3.2 to verify `useTranslations` + 4 tooltip_* keys (instead of literal-string check) / (4) extended AC3.5 to verify 4 NEW tooltip_* keys ko-KR SSOT parity. D-EPIC-16-REVIEW-DEFER-6 ✅ RESOLVED. |
