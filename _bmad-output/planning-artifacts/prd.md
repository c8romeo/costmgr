...
## Phase 3 (in-progress, 2026-08-20 — cj-style Phase 3 1번째 진입점 = Phase 3 PRD entry DONE 진입 wire = cj-style 49번째 epic 연속 정직 회복, Phase 3 wire 진입 대기 = cj-style Phase 3 2번째 진입점 = cj-style 50번째 epic 연속 정직 회복 진입 대기)

- **Auth Foundation (로그인/회원가입 UI + auth middleware = Epic 1 완성 territory 진입 결정)** — Phase 3 PRD entry DONE 2026-08-20 (cj-style Phase 3 1번째 진입점 = cj-style 49번째 epic 연속 정직 회복, master PRD v2.5 → v3.0 atomic edit). Phase 3 territory = Epic 1 carry-over 정직 회복 (Story 1.1 F-1 Supabase SSR client + F-4 accessToken string pass + F-30 rls_db fixture 모두 D-1-1-DEFER-* honestly DEFER preserved) + login/signup/forgot-password UI 신규 wire (Epic 1 partial scaffold = `(auth)` route group + `apps/web/app/[locale]/(auth)/onboarding/industry/page.tsx` + IndustrySelector 이미 wire 됨) + auth middleware EXTENSION (`apps/web/middleware.ts` 의 next-intl middleware EXTENSION = Supabase session check + `(dashboard)` 보호 + `?redirect=` 쿼리 보존 + `(auth)` 공개 route group bypass + `/api/v1/*` bypass + 2FA 미설정 사용자 `/account/security?reason=2fa_required` redirect) + logout flow + capability matrix v1.23 → v1.24 EXTENSION (LOGIN + SIGNUP + AUTH_MIDDLEWARE + FORGOT_PASSWORD + LOGOUT 5 NEW rows, industry-agnostic 4-industry grants ✅/✅/✅/✅, CR 12-1 L4 precedent 미러) + AD-26 Auth Foundation 신규 결정 (Supabase SSR + sb-access-token cookie session + next-intl middleware EXTENSION + auth route group (auth) + dashboard route group (dashboard) 보호 + Epic 12 2FA 게이트 보존). Epic 1 close-out 시점에 본 Phase 3 wire scope 결정 보존 (cj-style Phase 3 1~3번째 진입점 진입 시점에 본체 분석 후 결정 가능 결정 wire 보존). **Phase 3 PRD entry wire scope (master PRD v3.0 atomic edit)**: (1) front matter title v2.5 → v3.0 + changelog v3.0 entry / (2) §F15 신규 (F15.1 login UI + Supabase SSR auth client + sb-access-token cookie session / F15.2 signup UI + tenant 생성 flow + onboarding/industry handoff / F15.3 auth middleware EXTENSION next-intl middleware + Supabase session check + (dashboard) 보호 + (auth) 공개 route group / F15.4 logout flow + ko-KR SSOT logout error envelope + audit-first INSERT / F15.5 forgot-password UI + Supabase resetPasswordForEmail + token rotation + 2FA 게이트 보존 / F15.6 tests + wire scope T1~T8 결정) / (3) §8.1 M0-(d) login + M0-(e) signup + M0-(f) auth middleware 3 NEW 인수 불릿 / (4) §15 로드맵 Phase 3 row status 백로그 → in-progress (PRD entry DONE 진입 wire) / (5) §부록 A A65+A66+A67+A68+A69 신규 결정 표 / (6) AD-26 Auth Foundation 신규 결정 (Supabase SSR + sb-access-token cookie session + next-intl middleware EXTENSION) / (7) capability matrix v1.23 → v1.24 EXTENSION LOGIN + SIGNUP + AUTH_MIDDLEWARE + FORGOT_PASSWORD + LOGOUT 5 NEW rows (industry-agnostic 4-industry grants ✅/✅/✅/✅, CR 12-1 L4 precedent 미러). **Phase 3 진입 flow (cj-style 1~3번째 진입점 결정 보존)**: (1) cj-style Phase 3 1번째 진입점 = Phase 3 PRD entry (cj-style 49번째): ✅ DONE 2026-08-20 (master PRD v3.0 atomic edit) / (2) cj-style Phase 3 2번째 진입점 = bmad-create-story spec (cj-style 50번째): Phase 3 wire spec (T1~T8) 결정 wire + Epic 1 carry-over F-1/F-4/F-30 honestly RESOLVE 결정 wire 진입 대기 / (3) cj-style Phase 3 3번째 진입점 = bmad-dev-story atomic wire (cj-style 51번째): Phase 3 본체 wire T1~T8 atomic single sprint (T1 Supabase SSR client + T2 login page + T3 signup page + T4 auth middleware EXTENSION + T5 logout + T6 forgot-password + T7 capability v1.24 + T8 tests + 3중 게이트 FINAL CLEAN atomic commit). A19 cohesion pattern 9 surface EXTENSION PASS 결정 (auth surface NEW). estimated ~70 NEW vitest PASS + ~10 NEW pytest PASS + 0 NEW ruff + 0 regressions. **Phase 3 진입 flow (cj-style Phase 3 1~3번째 진입점 결정 보존)**: (1) cj-style Phase 3 1번째 진입점 = Phase 3 PRD entry (cj-style 49번째): ✅ DONE 2026-08-20 (master PRD v3.0 atomic edit) / (2) cj-style Phase 3 2번째 진입점 = bmad-create-story spec (cj-style 50번째): Phase 3 wire spec 결정 진입 대기 / (3) cj-style Phase 3 3번째 진입점 = bmad-dev-story atomic wire (cj-style 51번째): Phase 3 본체 wire T1~T8 atomic single sprint 진입 대기. Epic 1 close-out 시점에 결정 wire 진입: A45+A46 결정 wire 진입 (bundled into ONE Epic 14 carry-over sprint 결정 보존) + Epic 12 2FA 게이트 보존 (Epic 12 wire 정합). 결정 wire 일자: 2026-08-20.
