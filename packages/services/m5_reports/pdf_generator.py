...
Story 9.4 (Epic 9 4번째 진입점) surface:
  ...
  `handoff-2026-08-17-9-3-done.md` lock + 9-4 spec):
    - Report #21 (Cost Object Breakdown, 본 Story 9.4 진입점)
    - Report #15 (활동원가 내역서, 후속 진입점)
...
def _compose_report15_pdf(
    *,
    ...
    """Compose Report #15 (활동원가 내역서) PDF byte stream.

    Wire-ready placeholder (Report #15 = 후속 진입점). 동일 PDF 1.7
    envelope pattern (Report #21 wire 동일 surface 재사용). 9-4 spec
    결정 시점 payload schema wire (D-9-4-DEFER-2, A31+ 결정).
    """
    # Minimal placeholder — use Report #21 PDF surface 동일.
    # Report #15 wire 진입 시 payload schema 결정 + body_lines 확장.
    return _compose_report21_pdf(request=request)
