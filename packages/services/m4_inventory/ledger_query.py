...
def build_carry_chain_query() -> LedgerQuery:
    ...
    sql = (
        ...
        f"         {_COL_INSERTED_AT}, 1 AS depth "
        f"    FROM {_TBL_INVENTORY_LEDGER} "
        f"   WHERE {_COL_TENANT_ID} = :{PARAM_TENANT_ID} "
        f"     AND {_COL_PRODUCT_ID} = :{PARAM_PRODUCT_ID} "
        f"     AND {_COL_EVENT_TYPE} IN ('opening_carried', 'opening_carried_stale_overwrite') "
        f"     AND {_COL_PERIOD_KEY} < :{PARAM_PERIOD_KEY} "
        f"  UNION ALL "
        f"  SELECT e.{_COL_EVENT_ID}, e.{_COL_TENANT_ID}, "
        f"         e.{_COL_PRODUCT_ID}, e.{_COL_PERIOD_KEY}, "
        f"         e.{_COL_EVENT_TYPE}, e.{_COL_QTY}, "
        f"         e.{_COL_INSERTED_AT}, cc.depth + 1 "
        ...
        f"        to_char((to_date(e.{_COL_PERIOD_KEY} || '-01', 'YYYY-MM-DD') "
        f"                  - INTERVAL '1 month'), 'YYYY-MM') "
        f"   WHERE e.{_COL_EVENT_TYPE} IN ('opening_carried', 'opening_carried_stale_overwrite') "
        f"     AND cc.depth < {CARRY_CHAIN_RECURSION_DEPTH} "
        f") "
        f"SELECT {_COL_EVENT_ID}, {_COL_PERIOD_KEY}, {_COL_QTY}, "
        f"       {_COL_INSERTED_AT} "
        f"  FROM carry_chain "
        f" ORDER BY {_COL_PERIOD_KEY} DESC "
