...
class TestJITProvisioning:
    def test_tenant_not_found(self, jit_module) -> None:
    ...
    def test_full_flow(self, jit_module) -> None:
        async def _inner() -> None:
    ...
    def test_audit_first_insert_called(self, jit_module) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session(tenant_id=tenant_id)
            saml_attrs = _make_saml_attrs()
    ...
    def test_uses_correct_provider(self, jit_module) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session(tenant_id=tenant_id)
            saml_attrs = _make_saml_attrs()
