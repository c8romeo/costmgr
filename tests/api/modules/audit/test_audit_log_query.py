...
class TestAuditLogQueryFiltersValidation:
    ...
    def test_query_audit_log_rejects_filters_not_dict(self) -> None:
    ...
    def test_query_audit_log_rejects_bad_page(self) -> None:
        async def _inner() -> None:
            session = _make_session()
            with pytest.raises(AuditLogQueryInvalidFilterError) as exc_info:
                await query_audit_log(
    ...
    def test_query_audit_log_rejects_bad_page_size(self) -> None:
        async def _inner() -> None:
            session = _make_session()
            for bad in (0, 501):
                with pytest.raises(AuditLogQueryInvalidFilterError) as exc_info:
    ...
    def test_query_audit_log_rejects_date_range_inversion(self) -> None:
        async def _inner() -> None:
            session = _make_session()
            with pytest.raises(AuditLogQueryInvalidFilterError) as exc_info:
                await query_audit_log(
...
class TestAuditLogQueryPagination:
    ...
    def test_query_audit_log_returns_paginated_envelope(self) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session()
...
class TestAuditLogEntryLookup:
    ...
    def test_get_audit_log_entry_not_found(self) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session()
            empty = MagicMock()
    ...
    def test_get_audit_log_entry_returns_dict_envelope(self) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session()
            row = _make_row(
...
class TestCountAuditLog:
    ...
    def test_count_audit_log_returns_total(self) -> None:
        async def _inner() -> None:
            tenant_id = uuid.uuid4()
            session = _make_session()
            result = MagicMock()
...
class TestActivityStream:
    ...
    def test_query_activity_stream_rejects_invalid_window(self) -> None:
        async def _inner() -> None:
            session = _make_session()
    ...
    def test_query_activity_stream_bucket_granularity(
