...
| **D-CI-FUNC-4** (NEW, cj-215 관찰 — service-role-guard-lint job `Fail if service_role is invoked outside the guard module` step FAIL — **🔴 CRITICAL**, 실제 code violation, architecture integrity / multi-tenant security boundary 직접 위반, RLS bypass 위험) | ⚠️ **honestly DEFER + 🔴 CRITICAL PRIORITY** | Charlie + kjw | **cj-216 최우선** — service_role 호출 site 모두 grep + audit-first INSERT 패턴 적용 + lint script 검증 + pytest 회귀 결정 wire |
| **D-CI-FUNC-5** (NEW, cj-215 관찰 — web-e2e job `pnpm playwright install --with-deps chromium` step FAIL) | ⚠️ **honestly DEFER** | Amelia | cj-217 recovery sprint — `cd apps/web && pnpm playwright install --with-deps chromium` local 재현 + chromium system deps fix 결정 wire 보존 |
| **D-CI-FUNC-6** (NEW, cj-215 관찰 — smoke-e2e + rls-tests 2 jobs 의 `Install psql` step FAIL, 동일 root cause) | ⚠️ **honestly DEFER** | Charlie | cj-217 recovery sprint — ci.yml 의 psql install step 의 정확한 command 확인 + local 재현 + fix 결정 wire 보존 |
