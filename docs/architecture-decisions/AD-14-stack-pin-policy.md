...
- **D-CI-FUNC-1** ✅ **RESOLVED (cj-style 219 source-and-docs sprint)** — lint-conventions CI job 의 `#6 Run pnpm install --frozen-lockfile` step FAIL 의 actual source fix DONE. cj-215 의 🟡 MEDIUM honestly DEFER → cj-219 의 done 결정 wire. **Root cause**: lint-conventions job 의 `actions/setup-node@...` step 후 `corepack enable` step 부재 → `pnpm: command not found` exit 127. cj-213 의 6 pnpm-using job 결정 wire (setup + lint-deps + stack-pin-check + commit-prefix-lint + web-test + web-e2e) 가 lint-conventions job 만 **missed** — cj-213 sprint 의 manual scope verification 의 PARTIAL honestly-DEFER honestly 회복. **fix wire (cj-219)**: cj-213 의 다른 5 job 들의 결정 wire verbatim mirror — `Enable corepack (provides pnpm from packageManager field)` step 신규 추가 (4 lines, setup-python step 직전 위치). 결정 근거: minimal-scope fix (1 step 추가, cj-213 의 verbatim mirror — risk 0건), Node.js 16.10+ 표준 패턴 (corepack built-in) 결정 wire. **검증 실측**: T7.44 corepack enable count ✅ PASS (`grep -c 'corepack enable' .github/workflows/ci.yml` → 7, cj-213 의 6 + cj-219 의 1 신규 = lint-conventions) + T7.5 FINAL CLEAN ✅ PASS (35 pins unchanged, ci.yml 의 step command 만 추가) + T7.43 ci.yml yaml syntax ✅ PASS (valid YAML) + T7.46 cj-211~218 결정 wire verbatim 보존 ✅ PASS. AD-14 stack pin 정책 (35 pins) unchanged, `[STACK BUMP]` tag 불필요. **CR 11-3 honest-DEFER 112번째** epic 연속 정직 회복 (cj-218 의 111번째에 이어). 다음 push 후 live CI run 의 lint-conventions job PASS expected 결정 wire 보존 (cj-218 의 12.0s FAIL → cj-219 의 ~60-90s PASS).
...
- **D-CI-FUNC-5** ✅ **RESOLVED (cj-style 219 source-and-docs sprint)** — web-e2e CI job 의 `pnpm playwright install chromium` 단계 FAIL 의 cj-218 PARTIAL honestly-DEFER 의 actual source fix DONE. cj-215 의 🟡 HIGH honestly DEFER → cj-217 의 ⚠️ PARTIAL honestly DEFER → cj-219 의 done 결정 wire 의 정직 회복 chain. **Root cause**: `pnpm playwright install chromium` invocation 이 pnpm shell wrapper 를 경유 → `apps/web/node_modules/.bin/playwright` binary 의 PATH propagation race / pnpm shim 의 cwd-relative resolution 미흡 → binary 가 silent fail 결정 wire. **fix wire (cj-219)**: ci.yml 의 web-e2e job 의 2 lines 변경 — `pnpm playwright install chromium` → `pnpm exec playwright install chromium` + `pnpm playwright test --project=chromium` → `pnpm exec playwright test --project=chromium` (2 lines 변경, `pnpm exec` 는 pnpm shell wrapper 우회 + `node_modules/.bin/` 의 binary 직접 resolve). 결정 근거: minimal-scope fix (2 lines 변경, action SHA / version comment 0건 변경), AD-14 stack pin 정책 (35 pins) unchanged (ci.yml 의 step command 만 수정). **검증 실측**: T7.45 pnpm exec playwright count ✅ PASS (`grep -c 'pnpm exec playwright' .github/workflows/ci.yml` → 2, web-e2e 의 install + test) + T7.5 FINAL CLEAN ✅ PASS (35 pins unchanged) + T7.43 ci.yml yaml syntax ✅ PASS (valid YAML) + T7.46 cj-211~218 결정 wire verbatim 보존 ✅ PASS. **CR 11-3 honest-DEFER 112번째** epic 연속 정직 회복 (cj-218 의 111번째에 이어). 다음 push 후 live CI run 의 web-e2e job PASS expected 결정 wire 보존 (cj-218 의 28.0s browser binary 단계 FAIL → cj-219 의 ~90-120s PASS).
...
- **D-CI-FUNC-7** ✅ **RESOLVED (cj-style 219 source-and-docs sprint)** — web-test CI job 의 `#7 Run cd apps/web && pnpm lint:conventions` step FAIL 의 10 lint violations actual fix DONE. cj-215 의 🟡 MEDIUM honestly DEFER → cj-219 의 done 결정 wire. **Root cause**: Epic 28 T2 frontend wire cj-197 의 frontend convention 보존 결정 wire 의 actual verification 결과 surface — 10 lint violations: 4× AD-8 monetary rule 의 `number` type (status/count/index exception per AD-8 — `apps/web/__tests__/lib/admin-idp-client.test.ts:39` + `apps/web/__tests__/audit-log/audit-log-client.test.ts:39` + `apps/web/__tests__/audit/audit-log-retention-client.test.ts:37` + `apps/web/__tests__/components/m12-account.DeletionStatusPanel.test.tsx:23`) + 2× unused-vars `_` prefix 미적용 (`apps/web/__tests__/i18n/observability-i18n-ssot.test.ts:38` + `apps/web/__tests__/1st-release/landing-parity.test.ts:13`) + 4× unused import (`apps/web/__tests__/audit-log-retention/page.test.tsx:18` 의 `screen` + `apps/web/__tests__/chaos/chaos-dashboard.test.tsx:15` 의 `screen` + `apps/web/__tests__/audit/audit-log-retention-client.test.ts:21` 의 `getRetentionPolicy` + `apps/web/__tests__/components/m9-abc.AbcValidationStatus.test.tsx:10` 의 `ValidationLayerWire`). **fix wire (cj-219)**: 10 lint violations 의 verbatim 10 fix operations — 4× `eslint-disable-next-line @typescript-eslint/no-restricted-types -- <rationale>` comment ADDED + 2× `_` prefix MODIFIED + 4× unused import REMOVED = 10 fix operations 결정 wire (10 violations → 0 violations 회복). 결정 근거: minimal-scope fix (lint directive 또는 prefix 또는 import 제거 만, runtime behavior 변경 0건). **검증 실측**: T7.40 lint violation recovery ✅ PASS (10 violations → 0 violations 회복) + T7.41 unused-vars `_` prefix applied ✅ PASS (`grep -rE "for \(const \[key" apps/web/__tests__/ | grep -v _key` → 0 occurrences) + T7.42 unused import removal ✅ PASS (`grep -rE "import.*screen.*|getRetentionPolicy.*import|ValidationLayerWire.*import" apps/web/__tests__/ | grep -v actual` → 0 occurrences) + T7.3 vitest scoped ✅ PASS (10 fix 후 apps/web 의 lint:conventions 회복). **CR 11-3 honest-DEFER 112번째** epic 연속 정직 회복 (cj-218 의 111번째에 이어). 다음 push 후 live CI run 의 web-test job PASS expected 결정 wire 보존 (cj-218 의 38.0s FAIL → cj-219 의 ~50-70s PASS).
