...
- **D-CI-FUNC-5** ✅ **RESOLVED (cj-style 217 install-fix source sprint)** — web-e2e CI job 의 `#6 Run pnpm playwright install --with-deps chromium` step FAIL 의 actual env fix DONE. **fix wire (cj-217, Option A 채택)**: ci.yml 의 단일 step 을 2 step 으로 분리 — (a) `Install Playwright system dependencies` (explicit sudo apt-get install of 13 system libs: `libnss3 libnspr4 libatk1.0-0 libatk-bridge2.0-0 libcups2 libxkbcommon0 libxcomposite1 libxdamage1 libxrandr2 libgbm1 libpango-1.0-0 libcairo2 libasound2t64`) + (b) `pnpm playwright install chromium` (without `--with-deps`, apt-get 호출 분리되어 어느 stage 가 fail 하는지 surface). subprocess apt-get race condition 회피 + diagnostic surface 확보. AD-14 stack pin 정책 (35 pins) unchanged (ci.yml 의 step command 만 수정, `[STACK BUMP]` tag 불필요). 다음 push 후 live CI run 의 web-e2e job PASS expected 결정 wire 보존.
- **D-CI-FUNC-6** ✅ **RESOLVED (cj-style 217 install-fix source sprint)** — smoke-e2e + rls-tests 2 jobs 의 `#7 Install psql` step FAIL 의 actual env fix DONE. **fix wire (cj-217, Option A 채택)**: ci.yml 의 `Install psql` step (rls-tests + smoke-e2e 의 2 occurrences) 을 `apt-get update -qq && apt-get install -y -qq postgresql-client >/dev/null` → multi-line `sudo apt-get update -qq` + `sudo apt-get install -y --no-install-recommends postgresql-client` + `psql --version` verification 으로 교체. explicit sudo + stderr visible + install success verification. 2 jobs 공유 root cause → 1개 fix cycle 로 동시 RESOLVED 결정 wire. AD-14 stack pin 정책 (35 pins) unchanged. 다음 push 후 live CI run 의 smoke-e2e + rls-tests jobs PASS expected 결정 wire 보존.
